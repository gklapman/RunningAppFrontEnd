  ...
  render() {
    ...
    return (
      ...
      {this.state.snappedTesting && this.state.isRunning ?
        ...
        <View>
            <MapView.Polyline coordinates={snappedPosCoords} strokeColor={redish} strokeWidth= {10} />
            { unsnappedPosCoords.map((coord,idx)=>{
              // console.log('marker at ',coord,' idx at ', idx)
              return(<MapView.Marker coordinate={coord} title={JSON.stringify(coord)} key={''+idx} pinColor={redish}/>)
            })}
        </View> :
